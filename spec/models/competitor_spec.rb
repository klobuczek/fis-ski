describe Competitor, "#calculate" do
  it "returns 0 for no results" do
    c=Competitor.new
    allow(c).to receive(:season_results) { [] }
    expect(c.send(:calculate, :any_attr)).to eq(0.0)
  end

  it "sums up points for less than max results" do
    c=Competitor.new
    allow(c).to receive(:season_results) { (1..3).map { |p| double(:any_attr => p) } }
  ...
  it "sums up points for more than max results" do
    c=Competitor.new
    allow(c).to receive(:season_results) { (1..6).map { |p| double(:any_attr => p) } }
    allow(c).to receive(:season) { double(:max_races => 4) }
  ...
  it "should sort correctly" do
    ...
    r1.competitor.season_results=[r1, r2]
    r1.competitor.season_results.sort!
    expect(r1.competitor.season_results).to eq([r2, r1])
  end
end